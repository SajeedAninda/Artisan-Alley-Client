...
const OrderedProductForUser = () => {
    let axiosInstance = useAxiosInstance();
    let { loggedInUser } = useAuth();
    let currentUserEmail = loggedInUser?.email;

    const { data: myOrder, isLoading: isOrderLoading, refetch } = useQuery({
        queryKey: ['myOrder', currentUserEmail],
        queryFn: async () => {
            const response = await axiosInstance.get(`/getOrdersForUser/${currentUserEmail}`);
                ...
                <div>
                    {myOrder?.length === 0 ? (
                        <div className='h-[90%] flex justify-center items-center py-12'>
                            <h2 className="text-3xl font-bold text-[#442b20] text-center">No Orders / Bookings made</h2>
                        </div>
                    ) : (
                        <div className='py-6 w-full md:w-[80%] lg:w-[70%] mx-auto'>
                            {myOrder?.map(order => (
                                <div className='flex flex-col lg:flex-row gap-6 border-4 border-[#442b20] rounded-md p-8'>
                                    ...
                                    <div className="flex-1 text-center">
                                        ...
                                        <h2 className='font-bold text-xl text-[#442b20] mt-2'>Crafted By {order?.artisan_name}</h2>
                                        <h2 className='font-bold text-2xl text-[#442b20] mt-2'>Price: {order?.product_price}$/Piece</h2>
                                        <h2 className='font-semibold text-2xl text-[#442b20] mt-2'>Quantity:{order?.order_quantity}</h2>
                                        <h2 className='font-bold text-2xl text-[#442b20] mt-2'>Total Payable: ${order?.product_price * order?.order_quantity}</h2>
...
export default OrderedProductForUser;